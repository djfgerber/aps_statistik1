...
(c) Stellen Sie die Anzahl erinnerter Wörter in einem Histogramm mit dem `JJStatsPlot`-Modul und einer Korbbreite von $3$ erinnerten Wörtern dar. Wie viel Prozent der Menschen erinnerten sich an $8$ bis $10$ Wörter? Wie viele Menschen erinnerten sich an $8$ bis $10$ Wörter? Wieso stimmt hier die relative Häufigkeit `proportion` mit der absoluten Häufigkeit `count` überein?
...
tibble(x = exm_angst$x_means) %>% 
  ...
  labs(y = 'Häufigkeit', x = 'Durchschnittliche Angst')
```
...
plot_hist_means(exm_angst$x_means, exm_angst$lb_means_emp,  exm_angst$ub_means_emp, 0.5, 'Durchschnittliche Angst')
```
...
with(exm_vertraeglichkeit,
     plot_hist_means(means, lb, ub, 0.05, "Durchschnittliche Verträglichkeit"))
```
...
with(exm_angst,
     plot_hist_denstiy_expected_value(x_means, x_mean, x_sd, n, "Durchschnittliche Angst", 0.5))
```
...
with(exm_vertraeglichkeit,
     plot_hist_denstiy_expected_value(means, 4, sqrt((b - a) ^ 2 / 12), n, "Durchschnittliche Verträglichkeit", 0.05))
```
...
-->
Scrolle auf [seeing-theory.brown.edu > Continuous > Normal](https://seeing-theory.brown.edu/probability-distributions/index.html#section2) bis links 'Central Limit Theorem' sichtbar ist. 
...
a) Stelle $\alpha = 1, \beta = 1$, `Sample size = 3`. Wie viele Stichprobenziehungen braucht es, damit die Durchschnitte normalverteilt aussehen? 
b) Stelle $\alpha = 1, \beta = 1$, `Sample size = 10`. Wie viele Stichprobenziehungen braucht es, damit die Durchschnitte normalverteilt aussehen? 
c) Stelle $\alpha = 2, \beta = 5$, `Sample size = 3`. Wie viele Stichprobenziehungen braucht es, damit die Durchschnitte normalverteilt aussehen? 
d) Stelle $\alpha = 2, \beta = 5$, `Sample size = 10`. Wie viele Stichprobenziehungen braucht es, damit die Durchschnitte normalverteilt aussehen? 
e) Was kann daraus geschlossen werden?
...
a) $1000$ bis $2500$ Stichproben.
b) $150$ bis $250$ Stichproben.
c) $500$ bis $1100$ Stichproben.
d) $50$ bis $200$ Stichproben.
e) Je mehr Beobachtungen in einer Stichprobe, desto schneller ist der Durchschnitt normalverteilt. Je nach Stichprobenverteilung (Einstellung der $\alpha$ und $\beta$-Werte in der Simulation), ist der Durchschnitt schneller oder langsamer normalverteilt.
...
gen_exr_ki_mean_n_vary <- function(){
  ...
  samples <- sample_sizes %>%
    map(~rexp(.x,4))
  file_name <- '04-exr-stichprobengroesse.sav'
...
Eine Mensa will herausfinden, wie lange die Leute um 12h durchschnittlich anstehen müssen. Dazu befragt sie `r exr_ki_mean_n_vary$ki_table$n[1]` Kund:innen. Das Resultat der Untersuchung ist, dass die Kund:innen im Durchschnitt $0.4$ Stunden anstehen müssen. Leider ist das Konfidenzintervall sehr gross. Da die Mensa nicht weiss, wie viele Leute befragt werden müssen, um ein kleineres Konfidenzintervall zu erhalten befragt sie in 4 weiteren Runden jeweils `r knitr::combine_words(exr_ki_mean_n_vary$ki_table$n[-c(1)], and = " und ", oxford_comma = FALSE)` Kund:innen. Die Daten aller $5$ Untersuchungen sind unter `r inline_code(exr_ki_mean_n_vary$file_name)` abgelegt. Für jede der $5$ Stichproben:
...
Eine Klasse bringt bei einem Biologietest eine durchwachsene Leistung, siehe `r inline_code(exr_biologietest$file_name)`. Die Lehrkraft entscheidet sich die genau gleichen Test zu wiederholen. Berichten Sie das durchschnittliche Resultat der beiden Tests und schätzen Sie den Einfluss der Standardabweichung auf die Länge des Konfidenzintervalls ein.
:::
...
gen_exm_vermoegen <- function(){
  ...
  plot_hist_density <- plot_hist_denstiy_expected_value(
    ...
    n,
    "Durchschnittliche Vermögen [CHF]",
    5000)
  ...
  plot_hist_curve_pval_reja <- ggplot() +
    ...
    labs(x = "Durchschnittliche Vermögen", y = "W.-dichte                              Häufigkeit")
...
```{r exm-vermoegen-histogram-plot, fig.cap="Verteilung der durchschnittlichen Vermögen unter der Annahme, dass die Nullhypothese wahr ist.", echo = FALSE}
...
```{r exm-vermoegen-plots-sampling-theorie, fig.cap="Oben: Histogramm der simulierten Verteilung; unten: theoretische t-Verteilung; links: Illustration p-Wert; rechts: Illustration Ablehnungsbereich. Die Linie entspricht dem beobachteten Stichprobenmittelwert. Alle Verteilungen basieren auf der Annahme, dass die Nullhypothese wahr ist."}
...
gen_exm_alexithymie <- function(){
  ...
  plot_hist_curve_pval_reja <- ggplot() +
    ...
    line_xlim +
    plot_attr_reja_pval$theme +
    labs(x = "Durchschnittliche Alexithymie", y = "W.-dichte                              Häufigkeit")
...
```{r exm-alexithymie-plot, fig.cap="Oben: Histogramm der simulierten Verteilung der Alexithymie-Mittelwerte; unten: theoretische t-Verteilung; links: Illustration p-Wert; rechts: Illustration Ablehnungsbereich. Die Linie entspricht dem beobachteten Stichprobenmittelwert. Alle Verteilungen basieren auf der Annahme, dass die Nullhypothese wahr ist."}
...
```{r exm-breakup-hist-pval-reja, fig.cap="Trennungsschmerz: Ablehnungsbereich und p-Wert bei wiederholter Stichprobenziehung (oben), approximiert durch den t-Test (mitte) und approximiert durch den Welch-Test (unten). Alle Ablehnungsbereiche und p-Werte sind ähnlich."}
...
```{r exm-emotional-stroop-hist-pval-reja, fig.cap="Emotionaler Stroop-Test bei PTSD: Ablehnungsbereich und p-Wert bei wiederholter Stichprobenziehung (oben), approximiert durch den t-Test (mitte) und approximiert durch den Welch-Test (unten). Der Ablehnungsbereich und p-Wert des Welch-Tests bildet die wiederholte Stichprobenziehung genauer ab im Vergleich zum t-Test."}
...
::: {.exercise  #wordsrecall}
<!-- 
Ziel: 
- 
-->
In einem Experiment wird das Worterinnerungsgedächtnis von Patientinnen mit links (`LT Epilepsie`)- respektive rechtsseitiger (`RT Epilepsie`) Temporallappenepilepsie getestet. Dazu wird eine Anzahl Wörter eingeprägt, welche die Patientinnen nach einer kurzen Zeit wiedergeben müssen und die Anzahl erinnerter Wörter wird festgehalten. Bei der Analyse mit $t$-Test und Welch-Test mit `Jamovi` sind die Ergebnisse in Abbildung \@ref(fig:exr-wordsrecall-output) entstanden. Welche der folgenden Aussagen sind wahr, welche falsch.

```{r exr-wordsrecall-output, out.width='100%', fig.cap='Jamovi Eingabe.'}
knitr::include_graphics("figures/06-exr-wordsrecall-jmv-output.jpg")
```

a) Die Hypothesenstellung ist gemäss Testausführung einseitig.
b) Der gefundene Effekt ist gemäss Cohen als mittel einzustufen.
c) Der $p$-Wert des Welch-Tests nähert den tatsächlichen $p$-Wert am besten an.
d) Es gibt einen signifikanten Unterschied bei der durchschnittlichen Worterinnerungsleistung zwischen links- und rechtsseitiger Temporalepilepsie betroffener.
...
$N = `r exm_aufgeschlossenheit_jung_alt$n1`$ junge mit einer Aufgeschlossenheit von $M = 5.38, SD = 0.92$ und $N =`r exm_aufgeschlossenheit_jung_alt$n2`$ alte mit einer Aufgeschlossenheit von $M = 5, SD = 0.87$. Die Daten sind unter `r inline_code(exm_aufgeschlossenheit_jung_alt$file_name)` verfügbar. (Beispiel frei erfunden.)
:::
...
$$
p(\text{[alkohol > 1x pro Woche] und tertiär}) = \frac{`r exm_alcohol_edu$jmv_output_freq %>% filter(alkoholkonsum == "oft") %>% pull(all_of("2_count"))`}{`r exm_alcohol_edu$jmv_output_freq %>% filter(alkoholkonsum == "Total") %>% pull(all_of("total_count"))`} = `r round((exm_alcohol_edu$jmv_output_freq %>% filter(alkoholkonsum == "oft") %>% pull(all_of("2_count")))/(exm_alcohol_edu$jmv_output_freq %>% filter(alkoholkonsum == "Total") %>% pull(all_of("total_count"))),5)`
$$
...
\frac{
  `r exm_alcohol_edu$jmv_output_freq %>% 
    filter(alkoholkonsum == "oft") %>% 
    pull(all_of("total_count"))`
  }{
  `r exm_alcohol_edu$jmv_output_freq %>% 
    filter(alkoholkonsum == "Total") %>% 
    pull(all_of("total_count"))`} 
\cdot 
\frac{
  `r exm_alcohol_edu$jmv_output_freq %>% 
...
`r round((exm_alcohol_edu$jmv_output_freq %>% 
    filter(alkoholkonsum == "oft") %>% 
    pull(all_of("total_count")) / 
    exm_alcohol_edu$jmv_output_freq %>% 
    filter(alkoholkonsum == "Total") %>% 
    pull(all_of("total_count"))) * 
    (exm_alcohol_edu$jmv_output_freq %>% 
    filter(alkoholkonsum == "Total") %>% 
...
Der [**$\chi^2$-Vierfeldertest**]{.customdef #customdef-vierfeldertest}
...
<!--chapter:end:src/10-mehrfelder.Rmd-->

# Begriffsverzeichnis {-}

- [$\chi^2$-Mehrfeldertest](#customdef-mehrfeldertest)
- [$\chi^2$-Vierfeldertest](#customdef-vierfeldertest)
